/***
   ...
   *  @description
	...
	*	QQ交流群：536665479
	*
>>>>> V1.1 版本变更说明（2025-12-29）：
	*
	*	1.将SPI初始化独立出去，兼容cubeMX工程
	*	2.修正 LCD_DrawImage() 函数长宽不相等时，绘制出错的问题
	*
>>>>> 重要说明：
	*
	*  1.屏幕配置为16位RGB565格式
...
//extern	 SPI_HandleTypeDef hspi1;			// SPI_HandleTypeDef 结构体变量
...
{
	LCD_CS_L;
	LCD_SPI_WriteByte(lcd_data >> 8);  // 高8位
	LCD_SPI_WriteByte(lcd_data);       // 低8位
	LCD_CS_H;
}
...
*
*	函数功能: 批量写入数据到屏幕
*	说    明: 已改为软件SPI实现，删除了所有硬件SPI格式切换，其他代码不变
****************************************************************************************************************************************/
void  LCD_WriteBuff(uint16_t *DataBuff, uint16_t DataSize)
{
	...
	{
		LCD_SPI_WriteByte(DataBuff[i] >> 8);
		LCD_SPI_WriteByte(DataBuff[i]);
	}
...
/****************************************************************************************************************************************
*	函 数 名: SPI_LCD_Init
*	函数功能: 初始化软件SPI以及屏幕控制器的各种参数
*	说    明: 删除了所有硬件SPI相关代码，函数名和参数不变
****************************************************************************************************************************************/
...
void LCD_Clear(void)
{
	...
	{
		LCD_SPI_WriteByte(LCD.BackColor >> 8);
		LCD_SPI_WriteByte(LCD.BackColor);
	}
...
void LCD_FillRect(uint16_t x, uint16_t y, uint16_t width, uint16_t height)
{
   ...
   LCD_SetAddress( x, y, x+width-1, y+height-1);	// 设置坐标	
	...
	for(i=0;i<width*height;i++)				
	{
		LCD_SPI_WriteByte(LCD.Color >> 8);
		LCD_SPI_WriteByte(LCD.Color);
	}
